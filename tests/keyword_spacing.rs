...
use prec_bsl::text_fixers::{KEYWORD_SPACING_RULE, add_spaces_before_keywords};

#[test]
fn keyword_spacing_matches_golden_fixture_and_is_idempotent() {
    ...
    let input = read_fixture_text(
        "tests/fixtures/golden/ДобавлениеПробеловПередКлючевымиСловами/кириллический_модуль/input.bsl",
    );
    let expected = read_fixture_text(
        "tests/fixtures/golden/ДобавлениеПробеловПередКлючевымиСловами/кириллический_модуль/expected.bsl",
    );
    ...
    let config = keyword_spacing_config();
    ...
    assert_eq!(first_report.results[0].rule_id, KEYWORD_SPACING_RULE);
    assert_eq!(
...
#[test]
fn keyword_spacing_only_changes_export_after_closing_paren_in_code() {
    let input = concat!(
        "Процедура A()Экспорт\r\n",
...
fn keyword_spacing_config() -> prec_bsl::config::ResolvedConfig {
    parse_config_str(
        r#"{
            "Precommt4onecСценарии": {
                "ГлобальныеСценарии": ["ДобавлениеПробеловПередКлючевымиСловами.os"]
            }
        }"#,
    )
    .unwrap()
...
fn temp_repo(test_name: &str) -> PathBuf {
    ...
    let path = std::env::current_dir()
        ...
        .join("target")
        .join("keyword-spacing-tests")
        .join(format!("{}_{}_{}", std::process::id(), test_name, nonce));
    fs::create_dir_all(&path).expect("temporary keyword spacing test repo must be created");
    path
}
